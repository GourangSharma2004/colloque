...
export default function ZeroToOneSummaryPage() {
  const [isBookmarked, setIsBookmarked] = useState(false);
  const bookmarkId = "doc-zero-to-one";
  const docTitle = "Zero to One Summary";
  ...
  const handleBookmarkToggle = () => {
    ...
    if (isBookmarked) {
      ...
      updated = [
        { id: bookmarkId, label: docTitle, href: "/book-summaries/zero-to-one", lastRead: Date.now() },
        ...bookmarks,
      ];
    }
    localStorage.setItem("colloque_bookmarks", JSON.stringify(updated));
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100vh" }}>
      <iframe
        ...
        title="Zero to One — Full Summary"
      />
      <button
